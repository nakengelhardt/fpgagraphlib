...
class Scatter(Module):
    def __init__(self, pe_id, config, port=None):
        ...
        mem_size = max(2, len(config.adj_idx[pe_id]))
        mem_init = _pack_adj_idx(config.adj_idx[pe_id])
        # mem_init.extend([0] * (max_edges_per_pe - len(mem_init)))
        self.specials.mem_idx = Memory(edgeidsize*2, mem_size, name="edge_csr_idx", init=mem_init)
