...
class Network(Module):
    def __init__(self, config, pe_start, pe_end):
        ...
        ext_msg_channel_in = Array(self.external_network_interface_in[i].msg.raw_bits() for i in range(config.addresslayout.num_channels))
        ext_dest_pe_channel_in = Array(self.external_network_interface_in[i].dest_pe for i in range(config.addresslayout.num_channels))
        ext_valid_channel_in = Array(self.external_network_interface_in[i].valid for i in range(config.addresslayout.num_channels))
        ext_ack_channel_in = Array(self.external_network_interface_in[i].ack for i in range(config.addresslayout.num_channels))
        ...
        self.comb += [
            router.dest_pe_in.eq(ext_dest_pe_channel_in[network_round]),
            fifo_msg_in[router.sink_out].eq(ext_msg_channel_in[network_round]),
            fifo_dest_pe_in[router.sink_out].eq(ext_dest_pe_channel_in[network_round]),
            fifo_valid_in[router.sink_out].eq(ext_valid_channel_in[network_round]),
            ext_ack_channel_in[network_round].eq(fifo_ack_in[router.sink_out])
        ]

        # push to outside
        ext_msg_channel_out = Array(self.external_network_interface_out[i].msg.raw_bits() for i in range(config.addresslayout.num_channels))
        ext_dest_pe_channel_out = Array(self.external_network_interface_out[i].dest_pe for i in range(config.addresslayout.num_channels))
        ext_valid_channel_out = Array(self.external_network_interface_out[i].valid for i in range(config.addresslayout.num_channels))
        ext_ack_channel_out = Array(self.external_network_interface_out[i].ack for i in range(config.addresslayout.num_channels))
        ...
        self.comb += [
            [self.roundrobin.request[i].eq(fifo_valid_out[i]) for i in range(num_local_pe + 1)],
            self.roundrobin.ce.eq(1),
            roundpar.eq(fifo_round_out[self.roundrobin.grant]),
            ext_msg_channel_out[roundpar].eq(fifo_msg_out[self.roundrobin.grant]),
            ext_dest_pe_channel_out[roundpar].eq(fifo_dest_pe_out[self.roundrobin.grant]),
            ext_valid_channel_out[roundpar].eq(fifo_valid_out[self.roundrobin.grant]),
            fifo_ack_out[self.roundrobin.grant].eq(ext_ack_channel_out[roundpar])
        ]
