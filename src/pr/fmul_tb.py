    ...
    def test_mul_single(self):
        ...
        def gen_input():
            ...
            for a, b in testcases:
                ...
                yield self.tb.dut.ce.eq(1)
                yield
                while not (yield self.tb.dut.valid_o):
                    yield
                for _ in range(10):
                    yield
                with self.subTest(a=a, b=b):
                    self.assertAlmostEqual(convert_32b_int_to_float((yield self.tb.dut.r)), a*b, delta=1E-6)
        
        self.run_with(gen_input(), vcd_name="tb1.vcd")
