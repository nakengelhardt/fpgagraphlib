...
class Config:
    def __init__(self, adj_dict, use_hmc=False, share_mem_port=False, **kwargs):
        ...
        self.init_nodedata = [[convert_record_to_int(self.addresslayout.node_storage_layout, num_neighbors=(len(self.adj_dict[self.addresslayout.global_adr(pe_adr=pe, local_adr=node)]) if self.addresslayout.global_adr(pe_adr=pe, local_adr=node) in self.adj_dict else 0), sum=0, num_recvd=0, active=0)  for node in range(max_node[pe] + 1)] for pe in range(self.addresslayout.num_pe)]
