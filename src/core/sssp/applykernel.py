...
from sssp.interfaces import payload_layout, node_storage_layout
...
class ApplyKernel(Module):
    def __init__(self, addresslayout):
        ...
        self.comb+= [
            ...
            self.message_out.dist.eq(self.message_in.dist),
    ...
    def gen_selfcheck(self, tb, quiet=True):
        ...
        while not (yield tb.global_inactive):
            ...
            if (yield self.valid_in) and (yield self.ready):
                ...
                num_messages_in += 1
                if not quiet:
                    print("State in: {} / Message in: {} / {}update".format((yield self.state_in.dist), (yield self.message_in.dist), "" if (yield self.message_valid) else "no "))
            if (yield self.message_valid) and (yield self.message_ack):
                num_messages_out += 1
                if not quiet:
                    print("Node " + str((yield self.nodeid_out)) + " updated in round " + str(level) +". New distance: " + str((yield self.message_out.dist)))
            yield
