...
class UnCore(Module):
    def __init__(self, config):
        ...
        assert(config.addresslayout.num_ext_ports == 2)
        ...
        for core in range(2):
            for port in range(2):
                n = port*config.addresslayout.num_pe_per_fpga + (-1)**port * (config.addresslayout.num_pe_per_fpga//2)
                ext_message_width = self.cores[core].network.ext_message_width

                roundrobin = RoundRobin(n, switch_policy=SP_CE)
                self.submodules += roundrobin

                array_msg_out = Array(self.cores[1-core].network.efb.message_out_data[i*ext_message_width:(i+1)*ext_message_width] for i in range(port*(config.addresslayout.num_pe_per_fpga//2), port*(config.addresslayout.num_pe_per_fpga//2) + n))
                array_valid_out = Array(self.cores[1-core].network.efb.message_out_valid[i] for i in range(port*(config.addresslayout.num_pe_per_fpga//2), port*(config.addresslayout.num_pe_per_fpga//2) + n))
                array_ack_out = Array(self.cores[1-core].network.efb.message_out_ack[i] for i in range(port*(config.addresslayout.num_pe_per_fpga//2), port*(config.addresslayout.num_pe_per_fpga//2) + n))

                self.comb += [
                    [roundrobin.request[i].eq(array_valid_out[i]) for i in range(n)],
                    roundrobin.ce.eq(1),
                    self.cores[core].network.efb.message_in_data[port].eq(array_msg_out[roundrobin.grant]),
                    self.cores[core].network.efb.message_in_valid[port].eq(array_valid_out[roundrobin.grant]),
                    array_ack_out[roundrobin.grant].eq(self.cores[core].network.efb.message_in_ack[port])
                ]
